...
mod test {
    ...
    fn run_all_tests() {
        let test_cases = vec![
            TestCase { input: vec![8, 1, 5, 2, 6], expected: 11 },
            TestCase { input: vec![1, 2], expected: 2 },
            TestCase { input: vec![1, 3, 5], expected: 7 },
        ];

        for case in &test_cases {
            assert_eq!(max_score_sightseeing_pair(&case.input), case.expected, "{:?}", case)
        }
